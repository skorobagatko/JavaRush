    ...
    public void live() throws InterruptedException {
        Thread thread = new Thread() {
            ...
            private void someActions() throws InterruptedException {
                eat();
                play();
                Solution.this.sleep(1000);  // Здесь мы вызывали метод sleep, объявленный в классе Thread
            }                               // Чтобы вызывать тот метод sleep, который нам нужен (метод sleep внешнего класса-родителя)
        };                                  // нужно было обратиться к этому методу через объект внешнего класса
        thread.start();
        thread.join();
    }
