...
public class Solution {
    ...
    public static class Plane extends Thread {
        ...
        public void run() {
            boolean  isAlreadyTakenOff = false;  // самолет уже взлетел?
            while (!isAlreadyTakenOff) {  // пока самолет не взлетел
                if (RUNWAY.getTakingOffPlane() == null) {    //если взлетная полоса свободна
                    RUNWAY.setTakingOffPlane(this);  // передаем на взлетную полосу самолет (занимаем взлетную полосу)
                    System.out.println(getName() + " взлетает");
                    takingOff();//взлетает
                    System.out.println(getName() + " уже в небе");
                    isAlreadyTakenOff = true; // отмечаем самолет, как взлетевший
                    RUNWAY.setTakingOffPlane(null); // освобождаем взлетную полосу
